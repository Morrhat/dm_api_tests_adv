...
def test_put_v1_account_email():
    ...
    print(response.status_code)
    assert response.status_code == 200, f'Инициализация смены почты не удалась {response.json()}'
    ...
    response = login_api.post_v1_account_login(json_data=json_data)
    print(response.status_code)
    assert response.status_code == 200, f'Пользователь с новыми данными не авторизован {response.json()}'
